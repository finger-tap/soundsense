...
- **检测同步录音** -- 测量同时记录现场音频(AAC 24kHz)，历史可回放，报告可导出为含录音的 HTML 单文件；清除历史时录音文件一并清理
- **声源定位套件** -- 三点走位测试给出「倾向楼上/隔壁」推测与置信度(支持 Apple Watch 端走位、结果同步)；长时间监听在异常噪音超出背景基线时自动截取录音片段，按时间线标注噪音类型与位置推测(推测仅供参考)
